...
def load_default_dqn():
    return q_network_factory.make_simple_sequential(json.load(open("resources/" + conf.CONF.get_value("network"))))
...
def run(argv):
    ...
    if mode == "train":
        ...
        logger.warning("First argument must be either train, test or play.")
        return -1
